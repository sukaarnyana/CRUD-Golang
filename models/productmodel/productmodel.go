...
func Detail(id int) entities.Product {
	...
	SELECT p.id, p.name, c.id, c.name, p.stock, p.description, p.created_at, p.updated_at FROM products p LEFT JOIN categories c ON p.category_id = c.id WHERE p.id = ?
	`, id)
